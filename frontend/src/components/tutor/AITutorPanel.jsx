...
import { X, Send, Target, Maximize2, Minimize2 } from 'lucide-react';
...
// --- NEW: INTERACTIVE WIDGET COMPONENT ---
// --- UPGRADED: INTERACTIVE WIDGET COMPONENT ---
const InteractiveQuizWidget = ({ widgetData, onAnswerSubmit, disabled }) => {
  ...
  // We extract the core UI so we can render it in both sizes easily
    ...
    <div className={`w-full bg-white ${isExpandedView ? 'h-full flex flex-col justify-center max-w-4xl mx-auto p-6 md:p-12' : 'mt-3 border border-indigo-100 rounded-xl overflow-hidden shadow-sm'}`}>
      ...
      <div className={`${isExpandedView ? 'mb-8 md:mb-12' : 'bg-indigo-50 px-4 py-3 border-b border-indigo-100 relative'}`}>
        <div className="flex items-center justify-between mb-2">
          ...
          {/* Expand Button (Only visible in chat panel) */}
          {!isExpandedView && (
            <button 
        ...
        {widgetData.options.map((option, idx) => (
          <button
            ...
            onClick={() => {
              ...
              if (isExpandedView) setIsExpanded(false); // Auto-close modal when they answer!
            }}
  ...
  return (
    <>
      {/* 1. Standard View (Inside the Chat Panel) */}
      <WidgetContent isExpandedView={false} />

      {/* 2. Expanded Modal View (Pops out over the whole screen) */}
      {isExpanded && (
...
const AITutorPanel = ({ 
  ...
  const finalizeStreamingMessage = (payload) => {
    ...
    setMessages(prev => prev.map(item => item.id === id ? { 
      ...
      interactive_widget: payload.interactive_widget || null, // Capture the widget!
      streaming: false 
    } : item));
